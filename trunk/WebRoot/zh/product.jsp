...
<%
	...
	cmd = cmd==null||(cmd=cmd.trim()).length()==0?"List":cmd;
	int pageIndex = 1;
	try{ pageIndex = Integer.parseInt(request.getParameter("page")); }catch(Exception e){}
	int c = 0;
	try{ c = Integer.parseInt(request.getParameter("id")); }catch(Exception e){}
	String t = request.getParameter("Filed");
	int pt = 0;
	try{ pt = Integer.parseInt(request.getParameter("ddl_ptype")); }catch(Exception e){}
	int st = 0;
	try{ st = Integer.parseInt(request.getParameter("ddl_type")); }catch(Exception e){}
	String key = request.getParameter("txtName");
	key = key==null||(key=key.trim()).length()==0?"":key;
	
	String sql="SELECT serial,topic,img_url,date,typeid,id FROM t_product ";
	String wz = "";
	String sz = "";
	String bePost = "";
	
	if (!"topic".equals(t)&&!"content".equals(t))
	{
		if (c==0)
		{
			wz = "&gt;&gt;&gt;全部产品";
		...
		else
		{
			...
			try {
				...
				ResultSet rs = state.executeQuery("SELECT typeid,typename FROM t_product_type WHERE typeid=(SELECT pid FROM t_product_type WHERE typeid="+c+")");
				if (rs.next())
				{
					pid = rs.getInt(1);
					wz = "&gt;&gt;&gt;<a href='product.jsp?id="+pid+"'>"+rs.getString(2)+"</a>";
				}
				rs.close();
				state.close();
				...
				if (pid==0)
				{
					...
					rs = state.executeQuery("SELECT typename FROM t_product_type WHERE typeid="+c);
					if (rs.next())
					{
						wz = "&gt;&gt;&gt;<a href='product.jsp?id="+c+"'>"+rs.getString(1)+"</a>";
						sz = "&gt;&gt;&gt;所有小类";
					}
					rs.close();
					state.close();
					
					sql += "WHERE typeid="+c+" OR typeid IN(SELECT typeid FROM t_product_type WHERE pid="+c+") ORDER BY serial ASC";
				}
				else
				{
					state = con.createStatement();
					...
					if (rs.next())
					{
						sz = "&gt;&gt;&gt;<a href='product.jsp?id="+c+"'>"+rs.getString(1)+"</a>";
					}
					rs.close();
					state.close();
					
					sql += "WHERE typeid="+c+" ORDER BY serial ASC";
				}
	...
	else
	{
		...
		sql += " WHERE "+Util.getSafeStr(t)+" LIKE '%"+Util.getSafeStr(key)+"%'";
		if (st>0)
		{
			sql += " AND typeid="+st;
		}
		...
		bePost = "&Filed="+t+"&ddl_type="+st+"&ddl_ptype="+pt+"&txtName="+key;
	}
%>
				    ...
				    <tr valign="top">
				      ...
				      <td width="">
				      	<table width="100%" border="0">
								...
								<td>
									<%
										...
										try {
											...
											while (rs.next())
											{
												if (total/12==pageIndex-1)
												{
													count++;
									%>
											<a title="<%=rs.getString("topic")%>" class="prod" href="javascript:open_prod(<%=rs.getString("id")%>)">
												<img src="<%=rs.getString("img_url")%>" />
												<div style=" margin:0px 10px;"><%=rs.getString("topic")%></div>
											</a>
									<%
												}
												total++;
							...
							<tr>
								<td style="text-align:center;">产品共 <%=total %> 条 本页 <%=count %> 条 共 <%=total/12+(total%12!=0?1:0) %> 页 第 <%=pageIndex %> 页 <%=pageIndex>1?"<a href='product.jsp?page="+(pageIndex-1)+bePost+"'>上一页</a>":"上一页" %> <%=pageIndex<total/12+(total%12!=0?1:0)?"<a href='product.jsp?page="+(pageIndex+1)+bePost+"'>下一页</a>":"下一页" %> <input type="text" style="width:20px;" id="page" value="<%=pageIndex %>"/> <a href="javascript:window.location='http://localhost:8080/zh/product.jsp?<%=bePost.length()>=1?bePost.substring(1)+"&":"" %>page='+document.getElementById('page').value;">GO</a></td>
							</tr>
						</table>
					  </td>
					</tr>
