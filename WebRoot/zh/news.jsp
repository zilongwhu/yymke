...
<%
	int pageIndex = 1;
	try{pageIndex = Integer.parseInt(request.getParameter("page"));}catch(Exception e){}
	int c = 0;
	try{c = Integer.parseInt(request.getParameter("c"));}catch(Exception e){}
	
	String sql="SELECT id,topic,date,typename FROM t_news,t_news_type WHERE t_news.typeid=t_news_type.typeid";
	if (c>0)
	{
		sql += " AND t_news.typeid="+c;
	}
		...
		<center>
			  ...
			  <table width="100%" cellpadding="0" cellspacing="0" border="0">
			    <tr>
			      ...
			      <td><div style="position:relative; text-align:right; margin-top:55px; margin-right:8px;">
			        <span onclick="var strHref=window.location.href;this.style.behavior='url(#default#homepage)';this.setHomePage('http://www.yymke.com');" style="CURSOR: hand">设为首页</span> | <span style="CURSOR: hand" onclick="window.external.addFavorite('http://www.yymke.com','岳阳明科电气有限公司')" title="岳阳明科电气有限公司">收藏本站</span> | <a href="lxwm.jsp">联系方式</a></div></td>
			    </tr>
			  </table>
			</div>
			...
			<div class="cd style1">
			  <table width="100%" cellpadding="0"  cellspacing="0" border="0">
			    <tr>
			      <td width="210">&nbsp;</td>
			      <td><script type="text/javascript" src="js/menu.js"></script></td>
			      <td width="100"><a href="index.jsp" target="_blank"><img src="images/englisg.gif" width="100" height="30" border="0" /></a></td>
			    </tr>
			  </table>
			</div>
			    ...
			    <tr valign="top">
			      ...
			      <table width="100%" cellpadding="0" cellspacing="0" border="0">
			        <tr>
			          ...
			          <img src="images/p_s.gif" width="210" height="28" /></td>
			        </tr>
			        <tr>
			          <td style="text-align:right; line-height:180%; padding-left:8px;">
			           <table width="100%" border="0" cellspacing="3" cellpadding="0">
			          <tr>
			            <td valign="bottom"><select size="1" name="Filed" style="width:200px;" id="Filed">
			            <option selected value="topic">产品名称</option>
			...
			//-->
			</script><select size="1" name="ddl_ptype" id="ddl_ptype" style="width:200px;" onChange="FillSubClass(this.options[this.selectedIndex].value)">
			<option selected value="0">所有大类</option></select></td>
			          </tr>
			          <tr>
			            <td><select size="1"  name="ddl_type" id="ddl_type" style="width:200px;">
			...
			//-->
			          ...
			          <tr>
			            <td><input name="txtName" id="txtName" style="width:195px;" value="关键字" size="20" onfocus="if(this.value=='关键字')this.value='';" onblur="if(this.value=='')this.value='关键字';" /></td>
			          </tr>
			          <tr>
			            <td align="center"><input type="submit" value=" 搜   索 " onclick="Search()" style="width:200px;" /></td>
			          </tr>
			        </table>
			          </td>
			        </tr>
			        <tr>
			          ...
			          <img src="images/p_l.gif" width="210" height="28" /></td>
			        </tr>
			        <tr>
			...
			function getOutLine()
			{
				...
				for (i=0;i<(outlookbar.titlelist.length);i++)
				{
					...
					outline+="'><div name=outlookdivin"+i+" id=outlookdivin"+i+" style='overflow:auto;width:100%;'>";
			...
			{
				var outline;
				outline="<div id=outLookBarDiv name=outLookBarDiv style='width=100%;'>";//高度在这里设置
			...
			<table width="210" border=0 align="left" cellpadding=0 cellspacing=0 id=mnuList style="WIDTH:210px;">
			  <tr> 
			      ...
			      <td width=""><table width="100%" border="0">
			        <tr>
			          <td><div style="width:92% auto; height:20px; position:relative;background-image:url(images/n.gif); background-repeat:no-repeat; background-color:#E1E2E6; margin-left:3px;"></div></td>
			        </tr>
			        <tr>
			          <td style="text-align:right; line-height:200%; padding-right:8px;">
			          <%
			          	state = con.createStatement();
			          	
						rs = state.executeQuery("SELECT * FROM t_news_type ORDER BY typeid ASC");
						while (rs.next())
						{
							int typeid = rs.getInt("typeid");
							%>
							<a href="news.jsp?c=<%=typeid %>" <%=typeid==c?"style='color:red;'":"" %>><%=rs.getString("typename") %></a> <%=rs.isLast()?"":"|" %>
							<%
						}
						rs.close();
						state.close();
					%>
			          <table width="100%" border="0">
						<%
							...
							while (rs.next())
							{
								if (total/15==pageIndex-1)
								{
									count++;
						%>
			              <tr>
			                <td> 　<img src="images/news.gif" align="absmiddle"></td>
			                <td class="ntd"><span class="news"> <a onmouseover="this.style.color='#FF0000'" onmouseout="this.style.color='#000000'" onclick="open_news(<%=rs.getString("id")%>)",title="<%=rs.getString("topic")%>"><%=rs.getString("topic")%></a></span></td>
			                <td class="ntd"> 　</td>
			                <td height="25" class="ntd"> 　</td>
			                <td height="25"> 　</td>
			              </tr>
						 <%
								}
			            ...
			            <table width="100%" border="0">
			              <tr>
			                <td style="text-align:center;">新闻共 <%=total %> 条 本页 <%=count %> 条 共 <%=total/15+(total%15!=0?1:0) %> 页 第 <%=pageIndex %> 页 <%=pageIndex>1?"<a href='news.jsp?page="+(pageIndex-1)+"&c="+c+"'>上一页</a>":"上一页" %> <%=pageIndex<total/15+(total%15!=0?1:0)?"<a href='product.jsp?page="+(pageIndex+1)+"&c="+c+"'>下一页</a>":"下一页" %> <input type="text" style="width:20px;" id="page" value="<%=pageIndex %>"/> <a href="javascript:window.location='http://localhost:8080/zh/news.jsp?c=<%=c %>&page='+document.getElementById('page').value;">GO</a></td>
			              </tr>
			            </table>
			          </td>
			        </tr>
			...
			<table width="100%" height="100%" border="0">
			  <tr>
			    ...
			    <td width="130" style="text-align:center; line-height:300%;"><a href="http://www.miibeian.gov.cn" target="_blank"><img src="images/gv.gif" width="35" height="43" border="0" /></a><br />
			      <a href="http://www.miibeian.gov.cn" target="_blank">湘ICP05012675</a></td>
			  </tr>
			</table>
